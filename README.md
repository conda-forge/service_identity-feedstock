...
Development: https://github.com/pyca/service_identity
...
Once the `conda-forge` channel has been enabled, `service_identity` can be installed with `conda`:
...
```
mamba install service_identity
```

It is possible to list all of the versions of `service_identity` available on your platform with `conda`:
...
```
mamba search service_identity --channel conda-forge
```
...
mamba repoquery search service_identity --channel conda-forge

# List packages depending on `service_identity`:
mamba repoquery whoneeds service_identity --channel conda-forge

# List dependencies of `service_identity`:
mamba repoquery depends service_identity --channel conda-forge
```
